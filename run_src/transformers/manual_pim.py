...
def generate_dcg(cpu_serial_profile_file, cpu_parallel_profile_file, pim_profile_file):
    ...
    print("OMP LIST: ", len(omp_op_list))
    print(omp_op_list)
    ...
    print("PIM LIST: ", len(pim_op_list))
    print(pim_op_list)

    print("DCG NODE: ", NUM_OF_NODES + len(omp_op_list) + len(pim_op_list))
...
def run_cpu_parallel(args, all_inputs):
    ...
    return cpu_profile_file, cpu_profile_records, omp_op_list
...
def run_pim(args, all_inputs):
    ...
    return pim_profile_file, pim_profile_records, pim_op_list

def run_pim_manual(args, all_inputs):
    align_map = pim_helper(args.model)

    pim_profile_file = run_profile("pim", args.model, True, args.basic_optimization, 4, args.batch_size,
                               args.sequence_length, all_inputs, align_map)
...
def run(args):
    ...
    # cpu_parallel_profile_file, cpu_parallel_profile_records, omp_op_list = run_cpu_parallel(args, all_inputs)
    ...
    # pim_profile_file, pim_profile_records, pim_op_list = run_pim(args, all_inputs)
    ...
    # ####### PIM
    # #####################
    print("RUNNING PIM, CPU_P, CPU_S")

    pim_manual_profile_file, pim_manual_profile_records = run_pim_manual(args, all_inputs)
    lines = ["MANUAL PIM PROFILE RESULT\n"]
    lines.append("=" * 64)
    lines += parse_profile_results(pim_manual_profile_records, args.kernel_time_only, args.threshold)

    lines.append("-" * 64)
    lines += group_profile_results(pim_manual_profile_records, args.kernel_time_only, args.threshold)
